...
def transcript_segments_overlap(segment_one: str, segment_two: str) -> int:
    ...
    i = 0
    while i < n and segment_one[-(i + 1)] == segment_two[i]:
        i += 1
    return i
...
def find_normalised_word_overlap(text1: str, text2: str) -> int:
    ...
    i = 0
    while i < n and words1[-(i + 1)] == words2[i]:
        i += 1
    return i
    

def plausible_overlap(overlap: int, expected_overlap: int, min_percent: float, max_percent: float) -> bool:
    """
    ...
    if abs(min_percent) >= 1 or min_percent >= max_percent:
        raise ValueError("Min should be between 0 and 1 and max should be higher than min")
